...
export function generateTailwindColors(palette, alias) {
  return {
    colors: generateColors(palette, alias),
    variables: generateVariables(palette, alias),
  }
}

/** @type {import('./tailwind.d.ts').GenerateColors} */
export function generateColors(palette, alias) {
  const result = {}
  const vars = {}
  ...
  if (alias) {
    return { ...result, ...generateColors(alias(vars)) }
  }

  return result
}

/** @type {import('./tailwind.d.ts').GenerateVariables} */
export function generateVariables(palette, alias) {
  const result = {}
  const vars = {}
  for (const [name, color] of Object.entries(palette)) {
    if (typeof color === 'string') {
  ...
  if (alias) {
    return { ...result, ...generateVariables(alias(vars)) }
  }
